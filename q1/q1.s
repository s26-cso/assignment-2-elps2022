...
.equ NODE_VAL,   0
.equ NODE_LEFT,  8   # 4B val + 4B padding
.equ NODE_RIGHT, 16
.equ NODE_SIZE,  24  # 8+8+4 rounded up to 8-byte alignment
...
insert: #a0 contains the node, a1 contains the value to be inserted
    ...
    bnez a0,do_traversal   #if root is not NULL, proceed normally
    mv a0,a1               #empty tree: just make a node and return it
    call make_node
    ld ra,0(sp)
    ld s0,8(sp)
    ld s1,16(sp)
...
do_traversal:
    ...
    insertion:
        ...
        j done
    insert_left:
        sd t3,NODE_LEFT(t1)    #was a0 (bug), should be t3
    done:
